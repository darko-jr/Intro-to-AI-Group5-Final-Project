...
**Institution:** Ashesi University, Ghana (Class of 2028)

**Project Group:** Group 5 (Daniel Darko, Victoria Brown Afari , Esbert Borden, Vera Sedinam Dugah)
...
1. **High Academic Risk (G3 \ 9/20):** Student is forecasted to fail the course without immediate remedial intervention.
2. **Moderate Academic Risk (10 \ G3 \ 14/20):** Student is currently meeting minimum passing thresholds but exhibits vulnerability factors (e.g., attendance slippage or past failures).
3. **Low Academic Risk (G3 \ 15/20):** Student exhibits strong academic mastery and stability.
...
| Evaluation Metric | Baseline Logistic Regression | Baseline Decision Tree | Baseline Random Forest | Tuned Random Forest (Deployed) |
| :--- | :--- | :--- | :--- | :--- |
| **Test Accuracy** | 77.55% (Val: 78.35%) | 71.43% (Val: 78.35%) | 76.53% (Val: 78.35%) | **80.61%** (Val: 79.38%) |
| **Macro F1-Score** | 0.6720 (Val: 0.7299) | 0.5842 (Val: 0.7342) | 0.6481 (Val: 0.6961) | **0.7101** |
| **Weighted F1-Score** | 0.7640 (Val: 0.7812) | 0.7024 (Val: 0.7842) | 0.7512 (Val: 0.7706) | **0.7910** |
| **Critical False Negatives** | 0 Students (0.0%) | 3 Students | 1 Student | **0 Students (0.0% FN Rate)** |
...
> **Model Selection Rationale (Why Tuned Random Forest over Logistic Regression):**  
> While linear models like Logistic Regression provided a competitive baseline during initial exploration (78.35% validation accuracy), student academic risk exhibits pronounced non-linear thresholds (e.g. failing $G1 \le 9$ coupled with attendance slippage $\ge 8$ days). The Tuned Random Forest with balanced class weighting was selected because it effectively models non-linear feature interactions, achieves the highest test accuracy (**80.61%**), and guarantees zero critical misses on held-out student cohorts.
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **1** | `G1` | First Period Grade | 0 to 20 points | **44.68%** | Primary academic indicator. | 9 is failing cutoff. |
| **2** | `failures` | Past Course Failures | 0 to 3+ courses | **16.95%** | Historical academic struggles across previous years. |
| **3** | `absences` | School Absences | 0 to 40+ days | **10.82%** | Unexcused missed school days during current term. | 8 triggers alert. |
...
3. The simulator visualizes the **Delta trajectory reduction**, calculating the exact percentage reduction in academic failure probability.
...
3. The system automatically performs:
   * Automated header matching (e.g. `attendance`,`absences`,`grade1`,`G1`).
